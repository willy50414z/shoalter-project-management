        ...
        if "EcomRevamp" == ann.name:
            for annotation_var in ann.element:
                if "methodPath" == annotation_var.name:
                    ...
                    remark = annotation_var.value

    result = {}
    if len(revamp_paths) > 0:
        result["related_method"] = revamp_paths
    if len(remark) > 0:
        result["remark"] = revamp_paths
    return result


def get_revamp_related_change_list(diff_info, repo_dir):
    change_list = []
...
if __name__ == '__main__':
    ...
    print(f"hybris branch[{branch_name}]diff lines[{str(branch_diff)}]")
    ...
    for revamp_related_change in revamp_related_change_summary:
        for changeRow in revamp_related_change["revamp_related_info_list"]:
            if list(changeRow.keys())[0] not in branch_change_summary:
                branch_change_summary.append(changeRow)
    branch_change_summary = sorted(branch_change_summary,
